...
│   │                    #   canonical rules: custom-alerts.tf + seaweedfs-write-path (only where seaweedfs_enabled) + velero-backup
...
│   ├── logging/         # Loki 6.55.0 (single-binary, S3 via the crypt gateway) + Promtail (syslog)
│   ├── seaweedfs/       # local S3 store: seaweedfs_enabled = false on ALL sites since 2026-09-25 (retired); module kept for a future site
...
- **Synology iSCSI (`REDACTED_b280aec5`)**: Databases, metrics, stateful workloads (Prometheus, Loki, Postgres, Thanos)
...
- **Thanos**: **v0.42.4** (bumped from v0.37.2 in the 2026-08-16 mirror campaign, batch 3c) — Query (2 replicas) + Store (2 replicas, S3 via the crypt gateway, now with a startupProbe: a v0.42 index re-sync got probe-killed on GR without it) + Compactor. Bucket `thanos-nl`. GR store reached via ClusterMesh.
...
- **Loki**: Single-binary, 100Gi iSCSI, retention NL 14 d / GR 30 d / NO 7 d, chunks on Hetzner via the crypt gateway (all sites since 2026-09-25). Retention is only real while `LokiRetentionNotRunning` (tier 1) is quiet: until 2026-09-21 it had NEVER succeeded on NL or GR (see the reclaim-hardening bullet under Known Issues)
...
uncompressed bases); only meshsat-hub's 8.8 GiB was. **The NL and GR stores were DELETED on 2026-09-25** (operator override of the 2026-12-15 floor, Plan A on
IFRNLLEI01PRD-2887): GR Thanos/Loki moved to the gateway the same day (history dropped), Velero's read-only
`default` BSLs went with them, `seaweedfs_enabled = false` on all three sites (notrf01's local store followed the same day: its Thanos sidecars were found still shipping to no-s3, pods recreated and the 27 GB of blocks copied to Hetzner first),
...
- **SeaweedFS on NL/GR: RETIRED 2026-09-25** (IFRNLLEI01PRD-2887, § Off-estate). The 12 SeaweedFS Known-Issues entries (filer store on CNPG, S3 ingress ModSecurity/buffering, read canary, slot exhaustion, filer.sync offsite, filer memory, low-space deadlock -2052, capacity RCA -2831, reclaim hardening -2850, object corruption -2090, PUT tail -2375, cross-site replication) are verbatim in root `CLAUDE-archive.md` § "k8s Known Issues: SeaweedFS (retired 2026-09-25)". No site runs SeaweedFS any more (notrf01 followed the same day); they matter only if a store ever comes back.
...
- **Alert hygiene 2026-09-25** (all sites): `REDACTED_8cdf02da` ignores the post-restart zero gauge and gives weekly schedules 8 d; the logging CiliumNetworkPolicy admits Prometheus on 3500 (loki-canary TargetDown since 09-10); `REDACTED_50695a0e` ignores the event-driven `omoikane_unit_failures.prom`; the CNPG rules scope by `{site=""}` (their `cluster` label is the CNPG cluster name: two rules had never fired, one fired forever). The 19 frozen textfile producers on nlclaude01 are in `textfile_collector/stale-20260925/`; only the two P0-2 dead-man files stay.
- **kube-proxy on nlk8s-ctrl02 (LXC)**: crash-looped 8,960 times (22-25 Sep) on `open /proc/sys/net/netfilter/nf_conntrack_max: permission denied` (an LXC cannot write host sysctls). Fixed 2026-09-25 by `conntrack.maxPerCore: 0` + `min: 0` in the kubeadm-owned `kube-system/kube-proxy` ConfigMap (kube-proxy then leaves conntrack alone; Cilium runs REDACTED_fd61d0fe anyway). NOT in tofu: kubeadm regenerates that ConfigMap on `kubeadm upgrade`, re-apply it then. The kube-proxy DaemonSet itself is still "under review".
