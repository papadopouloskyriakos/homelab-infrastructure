...
- **Nodes**: 3 control-plane (4 CPU, 8GB) + 4 workers (8 CPU, **10 GiB each, the fixed standard since 2026-09-13**, IFRNLLEI01PRD-2684; they were 8 GiB until 2026-08-24 and then unevenly 16/10/10/12, see `pve/CLAUDE.md` Key VMs), all Ubuntu 24.04.4, IPs 10.0.X.X-12 (CP), .20-23 (workers). Worker placement: node01 pve01, node02+node03 pve03, node04 pve04. **ctrl02 is an LXC, on nl-pve01 since pve02 was powered off (2026-08-25)** (kernel = its host's kernel, 6.17.13-3-pve live 2026-09-13; every kubeadm operation on it needs `--ignore-preflight-errors=SystemVerification`); the other 6 are QEMU VMs (kernels live 2026-09-13: ctrl01/03 7.0.0-28, node01-03 7.0.0-30, node04 7.0.0-31)
...
**Done 2026-09-20 (NL !583, GR !163, NO !77):** `REDACTED_5adcfee4` (9327)
+ headless Service + ServiceMonitor, scraped on **NL**, because the cluster that
would otherwise report GR's replication health is the one we just declared
untrustworthy.

**The alert itself was deliberately NOT written.** The first reading off the new
metric contradicted the data (see the filer.sync bullet under Known Issues), so
shipping the obvious rule would have shipped a permanently-firing one. Split out
as IFRNLLEI01PRD-2880; resolve that before adding a rule.
...
| cert-manager role | issuer (`acme_issuer_enabled = true`: ACME + 20 Certificates + PushSecret) | consumer (`false`: ExternalSecret pulls `REDACTED_2812d784`) | consumer (`false`) |
...
- Cluster-local alerts ARE mirrored and canonical: `custom-alerts.tf` (17 rules — the 17th is `REDACTED_16872a9b`, 2026-09-20 IFRNLLEI01PRD-2874, in its own `custom-node` group: a cordoned node was unreportable because nothing referenced `kube_node_spec_unschedulable` and kube-prometheus ships no such rule, so nlk8s-node01 sat `Ready,SchedulingDisabled` for 32 h; the 16th is `REDACTED_50666b71`, OMOIKANE-1657 2026-08-23: an Argo Application whose `syncPolicy.automated` is removed still reads Synced/Healthy while merges stop deploying, which `ArgocdAppOutOfSync` is structurally blind to until the next merge; note both Argo rules were DECORATIVE until the same MR added the argocd metrics Services + ServiceMonitors — Prometheus held zero `argocd_*` series), `seaweedfs-write-path-alerts.tf` (3), `velero-backup-alerts.tf` (6) — byte-identical, firing per-site against each cluster's own Prometheus.
...
- **Loki**: Single-binary, 100Gi iSCSI, retention NL 14 d / GR 30 d / NO 7 d, SeaweedFS S3 for chunks. Retention is only real while `LokiRetentionNotRunning` (tier 1) is quiet: until 2026-09-21 it had NEVER succeeded on NL or GR (see the reclaim-hardening bullet under Known Issues)
- **Promtail**: Syslog receiver on LB .68:514 — all Docker containers send logs here
- **BGPalerter**: Monitors the AS214304 prefix for hijacks, route leaks, RPKI invalidity. **NL ONLY since 2026-09-20** (`bgpalerter_enabled`, GR/NO at `replicas 0`): all three clusters watched the *same* single prefix via the *same* global RIS Live feed and mailed the *same* address, so extra copies added no coverage and tripled every notification — the same single-site rule as `estate_scrape_enabled`. Gated on `replicas` not `count`, so re-enabling a site is a one-word tfvars flip with no state moves.
  ⚠ **Its syslog reporter MUST stay `transport: udp`.** BGPalerter 2.0.1 binds its syslog `error`/`close` handlers with `function` instead of an arrow, so `this.logger` is undefined; `syslog-client` arms a 10 s idle timeout on a TCP socket and the resulting uncaught TypeError exits the process **about 10 s after every alert it sends**. That crash-looped all three clusters for 2h20m on 2026-09-19 (54 mails). IFRNLLEI01PRD-2873.
  ⚠ **Image is pinned by DIGEST** (`sha256:7716926952c431ba...`), not by tag. The `v2.0.1` tag is a *different build* (`sha256:c65a1942...`), so "tidying" the pin to that tag silently swaps the running image.
...
- `custom-alerts.tf` (CANONICAL, byte-identical with GR, 15 rules): ContainerOOMKilled, REDACTED_879bd353, REDACTED_02123891, REDACTED_a8a7eee8, REDACTED_67797f17, CiliumAgentNotReady, REDACTED_b94e0389, REDACTED_e52ce3d8, NFSMountStale, NFSMountHighLatency, ArgocdAppDegraded, ArgocdAppOutOfSync, HighPodRestartRate, PodCrashLoopBackOff (adopted from GR), NodeOOMKill (host-level kernel OOM — catches non-pod/compose OOMs, IFRNLLEI01PRD-2414), **REDACTED_16872a9b** (`kube_node_spec_unschedulable == 1` for 30m, group `custom-node`, IFRNLLEI01PRD-2874 — 30m is deliberately longer than a kubeadm-upgrade or PVE-migration drain so planned maintenance does not alert; invert-tested before shipping: the metric read 19 series all 0 while `max_over_time(...[2h]) == 1` returned exactly the cordoned node).
...
- `meshsat-copycat-alerts.tf` (NL-ONLY, mirror-exempt, 1 ScrapeConfig + 8 rules, MESHSAT-984): outside-in watch of the meshsat.info copycat and the GitHub endpoint it advertises a curl-to-root-shell installer from. **The expressions are INVERTED against every other rule here: they alert when a URL that is currently 404 STARTS returning 200**, which needs no blackbox module change because `probe_http_status_code` is exported even when `probe_success` is 0. That was deliberate, see the blackbox caveat below. Paired with `meshsat-copycat-watch.py` (claude-gateway, cron on nlclaude01) whose textfile gauges cover the case a status probe cannot: they can change the install URL, or move forge. **This set carries `tier=1` and the two payload rules carry `page=sms`, diverging from the omoikane/meshsat-hub outside-in sets which say "No SMS by design"** — owner decision, because it fires essentially never and the first hours are when reporting to Safe Browsing and the registry does the most good.
- **BLACKBOX CAVEAT (MESHSAT-985, open).** All outside-in probing here (`omoikane-public`, `meshsat-hub-public`, `meshsat-copycat`) goes through ONE blackbox exporter that is **not managed by this repo and not in git at all**: a hand-run Docker container on nlclaude01 (`prom/blackbox-exporter:latest`, `10.0.X.X:9115`, no volume mount, no compose file). Its `/etc/blackbox_exporter/config.yml` exists only in the container's writable layer, and the MeshSat mTLS modules (`tls_sni_mqtt_hub`, `tls_sni_reticulum`, `tls_hello_reticulum`) were written straight into the running container on 2026-09-08. **A `docker rm` and recreate silently loses them and three alert sets go dark.** Prefer expressions that work with the stock `http_2xx` module over adding a module, until MESHSAT-985 puts this under version control.
...
| **ntfy quiet topic `alrt-quiet` (`tier="2"`+`critical`, since 2026-09-23)** — never rings, never SMS | `ThanosCompactHalted`, `REDACTED_4c435e3b`, `LokiRetentionNotRunning`, `REDACTED_875a0962` (standing "reclaimer dead-man" conditions, moved off tier 1 after the 09-22/23 storm), plus the pre-existing tier-2 PVE rules |
| **ntfy tier 1, storage (2026-09-21/23)** | `REDACTED_47595c82` (per-server write probe, the ROOT alert), `REDACTED_8ea3848e`, `REDACTED_cc66fa91`, `SeaweedFSWillFillSoon`, `REDACTED_439f78b0`, `REDACTED_b3f2fec6`, `REDACTED_8e8e28d2` — **inhibited to ONE page per incident**: while any of the first three fires, every other `category=~"storage-.*"` alert of that site is muted, and `NotWritable` mutes the other two |

**Bridge semantics since 2026-09-23 (IFRNLLEI01PRD-2850):** one push per firing episode, NO timed re-page while an alert keeps firing (it used to re-page every 6 h: four standing conditions = 16 pushes/day), a re-fire after a resolve pages only if 6 h (`PAGING_REPAGE_S`) passed since the last page, resolves are silent (priority 2). Between 21 Sep 23:07 and 23 Sep 20:36 the old logic sent 61 pushes for seven alerts that were one fault (volume-0 `emergency_ro`), and the operator disabled the app. A pager that rings for consequences gets switched off before the cause is found. Runbook: claude-gateway `docs/runbooks/paging-ntfy.md`.
...
- **Velero offsite for the two SeaweedFS sites = the NL↔GR filer.sync**, which replicates `/buckets/velero` and `/buckets/velero-gr` bidirectionally (verified live 2026-08-24). Do not repoint the BSLs cross-site — the mirror-map substitution cascade fights it and the sync already moves the data off-site. Corollary (bit us once): when a peer site's filer store is reloaded from dumps that predate a velero wipe, the sync resurrects the wiped kopia tree on the other side — wipe BOTH sides after such a load.
  - **It got its first telemetry ever on 2026-09-20 (IFRNLLEI01PRD-2879/-2880), and the first reading contradicts the data — do NOT write a freshness rule yet.** Until then filer.sync exported nothing: no `-metricsPort` (the `weed` binary supports `-metricsIp`/`-metricsPort`; nothing passed them), no Service, no ServiceMonitor, `prometheus.io/scrape = "false"`, and a liveness probe of `pgrep -f filer.sync` that proves only that the process exists. Now `REDACTED_5adcfee4` (default 9327) + a headless Service + a ServiceMonitor, scraped **locally by NL** — the site that would otherwise have to report GR's replication health is GR, and GR is now NON-PRODUCTION. **Only NL runs filer.sync** (single bidirectional instance; GR and NO have `REDACTED_4bbaa453 = false` and carry the variable purely so the mirror holds).
  - ⚠ **`SeaweedFS_filerSync_sync_offset` is a REPLAY cursor, not a lag measure.** First reading: **27.7 days behind wall clock**, GR→NL moving **0 s in 240 s**, NL→GR at **0.19x realtime** (so it loses ground every cycle) — while the data was demonstrably **current**, `weekly-backup-20260920030046` present in `/buckets/velero-gr/backups/` on both sites. Both cursors sit on the `filer_sync_{a,b}_from_ts_ms` floor `1787516819348` = 2026-08-23T20:26:59Z, the -2605 cutover floor, **which also means every restart replays from that date and the window grows without bound**. The obvious `(time() - sync_offset/1e9) > 1h` would fire permanently, so it was measured and deliberately **not shipped**. Resolve IFRNLLEI01PRD-2880 before writing any rule here, and prove replication against the destination bucket rather than the cursor.
...
- **SeaweedFS low-space deadlock (IFRNLLEI01PRD-2052, 2026-07-30)** — took the whole NL S3 write path down for 11h13m and `cv.omoikane.coach` off the public internet. **Below `minFreeSpacePercent` a volume server marks every volume read-only AND refuses to compact**, so `volume.vacuum` — the tool that reclaims space — is disabled by the condition it would fix. `volume-1` sat at 6.99% free against the chart default of 7, i.e. ~49 MB short. Verified: vacuum selected zero volumes even at `-garbageThreshold=0.001`. ⚠ **Superseded on SeaweedFS 4.44 (drill 2026-09-21, IFRNLLEI01PRD-2850): a sweep DOES compact volumes that are read-only only because the disk is low**, as long as free >= that volume's .dat+.idx; see the reclaim-hardening bullet below for what actually bites now.
...
- **SeaweedFS capacity RCA (IFRNLLEI01PRD-2831, 2026-09-10) — the disk was never the root cause.** Second 95 %-full write outage in six weeks (cv.omoikane.coach down, Velero/Thanos/Loki/CNPG writes discarded). Measured per collection: **thanos-nl 883 GB raw +13 GB/day because the NL Thanos compactor had been HALTED since ~27 Aug** (`thanos_compact_halted=1`; retention is applied by the compactor, so nothing was deleted or downsampled for two weeks — it keeps syncing metadata every minute and looks alive); **cnpg-omoikane 367 GB +9 GB/day because `omoikane-main-daily` had not scheduled since 26 Aug** (a Backup stuck in `walArchivingFailing` blocks the next schedule; CNPG prunes WAL only past the newest RETAINED base backup, so `retentionPolicy 14d` pruned nothing); cnpg-litellm 235 GB at 14 d steady state; **loki 236 GB with retention configured but never provable — Loki was not scraped** (chart default `monitoring.serviceMonitor.enabled=false`), and **no CNPG Cluster in the estate exported metrics** (`monitoring.enablePodMonitor` unset). `replication 001` doubles every byte; slots were 1314/1332 of `-max 1400`. Every existing SeaweedFS rule fires *after* volumes are read-only, and no rule watched a reclaimer. Fix (MR !541 + twins GR !144 / NO !48, all canonical): `thanos_retention_raw` 14d (NL), weekly `seaweedfs-vacuum` CronJob (`namespaces/seaweedfs/vacuum-cronjob.tf` — the master's background GC never runs below `minFreeSpacePercent` nor on volumes under the 0.10 threshold), CNPG PodMonitor + Loki ServiceMonitor, and `namespaces/monitoring/seaweedfs-capacity-alerts.tf`: `ThanosCompactHalted`, `REDACTED_daccd521`, `REDACTED_2b95b756`, `REDACTED_2a56445c`, `CnpgMetricsMissing`, `LokiRetentionNotRunning`, `SeaweedFSVacuumJobNotRunning`, **`SeaweedFSFreeSpaceForecast` (`predict_linear` 14 d — the leading indicator that did not exist)**, `REDACTED_9873c1d0` (>10 % of capacity in 30 d, calibrated so thanos-nl/cnpg-omoikane/cnpg-litellm all trip on the 09-10 data), `REDACTED_2f6b6876`. **The rule: no consumer may write to this S3 without a retention that bounds it AND an alert proving the retention ran.** Still owed: `REDACTED_0a7b20f8` back to 7 once usage < 70 % (variables.tf:47); omoikane CNPG `retentionPolicy` 7 d + `enablePodMonitor` in the omoikane app repo; `etcd-snapshots-no` S3 prefix pruning (notrf01 `etcd-snapshot-ship.sh` prunes local disk only). Runtime remediation executed the same night (operator-approved), **with a relapse worth knowing:** (a) the stuck 26-Aug Backup was deleted, the schedule back-filled 08-27 and an on-demand backup completed in 4 min — retention then pruned five stale base backups; (b) `-minFreeSpacePercent` was lowered live 5 → 3 so vacuum could run, and the `seaweedfs-vacuum` CronJob was run by hand (0.10) plus one-off passes at 0.03/0.02; (c) **at 21:33 UTC volume-0 re-locked at exactly the 3 % line** — the un-halted compactor was writing compacted output while its 84 expired source blocks waited out `--delete-delay=48h`, eating the reclaim → `thanos_delete_delay` 48h → **2h** at NL (MR !543) + `thanos tools bucket cleanup --delete-delay=0s` as a one-off Job (compactor image, `REDACTED_5f4971dc` secret). **That tool is strict and refused on 5 blocks with an unreadable `meta.json`** (`01KDW8W7…`, `01KG4HHS…`, `01KH8SYD…`, `01KJD4KV…`, `01KKGY5X…`: index + EOF meta + EMPTY `chunks/` — aborted-upload debris of the -2090 corruption class, and almost certainly what halted the compactor in August); deleted via filer `fs.rm -r /buckets/thanos-nl/<ulid>` from a `weed shell` Job, after which cleanup deleted **100** blocks; floor then 3 → **2** (MR !544). Reclaimable garbage 298 GB raw, vacuum ~15 volumes/min at `-compactionMBps=50` — reclaim lags deletes by an hour or more, so **do not raise the floor until `kubelet_volume_stats_available_bytes` is comfortably above 10 % on BOTH PVs** (then 5, then 7). A `thanos_compact_halted` that clears on restart but the cleanup tool still errors = corrupt-meta debris; `fs.ls -l` shows the empty chunks dir. Note `volume.deleteEmpty -force` is deprecated in 4.44 (`-apply`), cosmetic.
- **SeaweedFS reclaim hardening (IFRNLLEI01PRD-2850, 2026-09-21), after the THIRD nl-s3 fill.** 03:10 lock -> cv down 07:18-17:13 (all times NL). Cause: the 09-15 park of the Thanos compactor ("back to 1 once there is room") and two CNPG Backups latched in `walArchivingFailing` since 09-11 were never undone; ~21 GB/day with no reclaimer. The forecast fired Sat 18 Sep 17:23 and paged nobody. What exists now, all canonical except the NL-only rule noted, all drilled:
  - **Paging (tier 1):** `SeaweedFSWillFillSoon` (6h trend -> floor within 72h; backtested on Thanos: would have paged Sat 19 Sep 06:00), `REDACTED_cc66fa91`, `REDACTED_8ea3848e`, `REDACTED_439f78b0`, `REDACTED_b3f2fec6` (85 %), `REDACTED_875a0962`, `REDACTED_4c435e3b`, `LokiRetentionNotRunning` (selector was `cluster=""`, but Loki exports `cluster="loki"`: it matched nothing and hid that NL/GR retention had NEVER run, blocked by expired index tables holding 0-byte files; removed), `REDACTED_8fdc9a1c` (NL-only file, NL+NO), `REDACTED_8e8e28d2`.
  - **`seaweedfs-reconciler`** (`namespaces/seaweedfs/reconciler*`, hourly, replaces the weekly vacuum): pauses the master's own vacuum walk, vacuums by explicit `-volumeId` largest-garbage-first (only what fits, skipping under-replicated volumes the master refuses), VERIFIES by re-reading `volume.list`, then deleteEmpty, `vacuum.enable` (always, in `finally`), `s3.clean.uploads`, quotas, orphan read-only path-rule cleanup, and drift (live floor / compactor replicas vs Git). ⚠ Four traps it encodes, each found live: `weed shell` exits 0 on errors AND prints NOTHING when the master refuses a vacuum ("Vacuum is already running" is in the master log only), so vacuum is judged by effect; the paused walk only stops between volumes (an 8 GB compaction held the lock 62 s); the master silently skips volumes with fewer copies than their placement ("not enough copies"; NL 3, NO 8); a 212 MB binary copy OOMs a 64 Mi init container on GR's slower disks.
  - **Drills (disposable SeaweedFS 4.44, 2 GiB, 40 % floor):** below the floor it dug out (35 % -> 75 % free, writable within a minute); 100 % full it FAILS loudly (nothing can compact); a master-flagged read-only volume is skipped by a sweep and compacted by the reconciler. ⚠ The floor is checked periodically: a fast writer overshot 40 % straight to 100 %.
  - **`_core/cnpg-janitor`** (hourly, every CNPG cluster): releases Backups latched `walArchivingFailing` > 2h or in flight > 12h (re-read + `resourceVersion` precondition) and FAILS when a ScheduledBackup has not fired, or its cluster not backed up, within 30h. Checks both: on-demand backups made `lastSuccessfulBackup` fresh while `omoikane-main-daily` had not fired since 09-12. Drill: first run failed naming exactly omoikane-main-daily/litellm-daily.
  - **Quotas** (`REDACTED_fd6d5350`, reconciled): NL thanos-nl 400 GiB + loki 150 GiB, NO thanos-no 40 + loki-no 20, GR none. ONLY buckets excluded from filer.sync. At 100 % a Thanos/Loki bucket also stops its OWN retention, hence the 85 % page. Drill on NO: 150 % -> refused within the enforcement tick, recovered 53 s after deleting. ⚠ `s3.bucket.delete` leaves the bucket's read-only path rule behind (upstream); the reconciler removes such orphans.
  - **`temporary_overrides`** (tfvars, all sites) + `k8s/tests/lint_temporary_overrides.py` in `validate_k8s_opentofu`: a floor < 5 % or a parked compactor fails CI without a dated override (<= 120 days); `REDACTED_8e8e28d2` pages after the date. GR registers its park until 2026-12-20. The same job now runs the janitor/reconciler/lint unit tests (59).
  - **Thanos:** `--compact.skip-block-with-out-of-order-chunks`; `REDACTED_dab4956a`, `REDACTED_1739a474` (warnings). ⚠ **Never run `thanos tools bucket retention` on a bucket whose compactor has been down** without `tools bucket inspect` first: it does not downsample, and on 09-21 it deleted NL raw 08-25 -> 09-14 that had no 5m/1h copy. That history is gone.
  - Also 09-21: omoikane-db 7-day retention + gzip base backups + PodMonitor (OMOIKANE-1669); reactive-resume probes off S3 (OMOIKANE-1668); etcd-snapshots-no pruned after 14 d (NO repo edge scripts).
  - **⚠ 2026-09-22/23: the first thing the hardened loop met was a read-only volume server, and it could not name it.** 22 Sep 05:17 NL reconciler run → 05:19/05:31 both volume servers OOM-killed (4 GiB limit; causal link unproven, timing exact) → 06:16 volume-0's ext4 `emergency_ro` (mount prints `rw`; `node_filesystem_readonly` 0). Every compaction then failed on volume-0 (`open ...cpd: read-only file system`, master cleans up the volume-1 side), so the reconciler reported `none of the top volumes compacted` hourly with 470 GiB garbage; replication 001 writes half-succeeded and pinned volume-1 at the floor; Thanos/Loki/canary/Hub WAL all failed behind it. Found by hand on 23 Sep 19:30 NL from the volume-0 log; cured by deleting the pod (kubelet remounted clean, write test ok; fallback: cordon + delete + 6 min podless). **Shipped (NL !592, GR !167, NO !81):** `namespaces/seaweedfs/write-probe.tf` (master assign pinned per data node + direct 4 KiB upload, every 10 min; an assign REFUSAL is not a verdict, an upload 500 is) → `REDACTED_47595c82` tier 1 + `SeaweedFSWriteProbeStale`; Alertmanager inhibition (one storage incident = one page); the four reclaimer dead-man rules to `tier = "2"` (quiet topic). Reading `[vacuum] FAILED: none ... compacted` in the reconciler log: check the volume servers' logs for `read-only file system` BEFORE suspecting the lock or the space. Same night: `thanos-compactor-0`'s PV was read-only from the same storm (halt reason `write chunks: sync ...: input/output error` on its OWN disk, not S3); floor 5 -> 3 % until 2026-09-30 (!593) then pod delete; the compactor cleaned 12 marked blocks in 3 s and resumed. Reclaim pace measured: ~4 min per 2 GB volume per server (syno01 iSCSI), ~200 MB back each; the loop cannot outrun ingest, so the floor override is what keeps writes alive until the garbage is gone.
- **SeaweedFS object corruption → Velero (IFRNLLEI01PRD-2090/-2091, fixed 2026-07-30)** — the write-path outage left corrupt objects behind, and the read side had no monitoring at all, so it surfaced only by accident. **All 14 Velero `kopia.maintenance` blobs were bad, in two different ways:**
  ...
  - **Both gaps closed since (line corrected 2026-09-24; it still said "nothing alerts"):** Velero backup-phase alerting shipped 2026-07-31 (MR !440/!441, `namespaces/monitoring/velero-backup-alerts.tf`, six gauge rules keyed on `velero_backup_items_errors`, never the resetting counters), and the SeaweedFS **read** path is watched by the read-path canary (`namespaces/seaweedfs/read-canary.tf`, see Known Issues above). At the time of the outage every check watched writes, so a 0-byte response to a 1890-byte object produced no signal.
...
- **Kubernetes MCP `node_management` drain is broken on kubectl 1.36 (found 2026-09-13).** It passes `--delete-local-data`, which kubectl removed, so every drain (even `dryRun`) fails with `unknown flag`. Drain with `kubectl_generic` instead: `command: drain`, `name: <node>`, flags `ignore-daemonsets: true`, `delete-emptydir-data: true`, `timeout: 600s` (used for node04's 12 -> 10 GiB resize; long-graceful pods such as prometheus push the call into the background, wait for its completion). `cordon` / `uncordon` via `node_management` still work.
- **NL velero kopia-maintain job loops are still live (rechecked 2026-09-13, IFRNLLEI01PRD-2793).** `awx-`, `monitoring-` and `pihole-default-kopia-maintain-job-*` pods end in `Error` about every 5 min on whichever worker they land; they did not stop after the 08-26 maxVolumes fix. The `monitoring` repo has a corrupt index blob (IFRNLLEI01PRD-2396). Do not read a pile of `Error` pods on one worker as that worker's fault.
