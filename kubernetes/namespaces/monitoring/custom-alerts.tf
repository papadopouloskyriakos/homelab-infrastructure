...
#
# CANONICAL (NL<->GR mirror, 2026-08-16): this file is byte-identical in both
# repos and holds only cluster-scope rules. Estate-scope rules (Pacemaker,
# Omoikane ClamAV/restic/systemd) live in estate-alerts.tf — NL repo only,
# mirror-exempt. The 3 legacy counter-based Velero rules were DELETED here:
# superseded by the gauge-based rules in velero-backup-alerts.tf (both sites),
# see the report / MR description.
# =============================================================================
        ...
        },
        {
          ...
          rules = [
            ...
            },
            {
              ...
              annotations = {
                summary     = "Pod {{ $labels.pod }} ({{ $labels.namespace }}) is in CrashLoopBackOff"
                description = "Container {{ $labels.container }} in pod {{ $labels.pod }} namespace {{ $labels.namespace }} has been in CrashLoopBackOff for more than 10 minutes. Check container logs for the crash reason."
              }
            }
          ]
        }
