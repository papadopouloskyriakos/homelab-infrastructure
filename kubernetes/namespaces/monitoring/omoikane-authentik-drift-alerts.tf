...
resource "kubernetes_manifest" "REDACTED_89c3fafb" {
  manifest = {
    apiVersion = "monitoring.coreos.com/v1"
    kind       = "PrometheusRule"
    metadata = {
      name      = "REDACTED_1f8b8b91-alert-rules"
          ...
          rules = [
            ...
            },
            {
              ...
              annotations = {
                ...
                description = "omoikane_authentik_drift_check_ts on {{ $labels.instance }} is >10 days old, so the weekly derived-image drift check is dark and the two rules above are running on stale data that still reads as healthy. Check: systemctl status REDACTED_1f8b8b91.timer / journalctl -u REDACTED_1f8b8b91.service on the host. NOTE the timestamp is stamped ONLY on a successful upstream query, so an unreachable GitHub also surfaces here rather than being mistaken for no-drift."
              }
            },
            {
