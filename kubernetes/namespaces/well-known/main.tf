...
# -----------------------------------------------------------------------------
resource "REDACTED_a9df2e77_v1" "well_known_files" {
  metadata {
  ...
  spec {
    ...
    template {
      metadata {
        ...
        annotations = {
          "checksum/config" = sha256(jsonencode(REDACTED_a9df2e77_v1.well_known_files.data))
        }
      }
      ...
      spec {
        ...
        volume {
          name = "config"
          config_map {
            name = REDACTED_a9df2e77_v1.well_known_files.metadata[0].name
            items {
              key  = "nginx.conf"
              path = "nginx.conf"
            }
