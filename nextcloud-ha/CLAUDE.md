...
### Layer 4: Database — moved to [`../dbcluster/CLAUDE.md`](../dbcluster/CLAUDE.md)

The Galera MariaDB + ProxySQL layer (nlcl01mariadb01/02, nlcl01garbd01,
nlproxysql01/02) is **shared infrastructure** — Nextcloud, paperless and CiviCRM all use
it — so since 2026-08-28 its documentation and config snapshots live in
[`native/dbcluster/`](../dbcluster/CLAUDE.md), not here. That doc carries the load-bearing
rules (single-writer `max_writers=1`, the `wsrep_auto_increment_control=OFF` data-integrity
hazard, the ProxySQL SQLite-not-cnf trap, the cnf include-order table).

What Nextcloud needs to know: it connects to `proxysql.example.net:6033` (DNS RR
.152 + .154, both proxies ACTIVE, NOT via HAProxy).
...
.154 — Nextcloud connects to both proxies directly, there is no standby).
